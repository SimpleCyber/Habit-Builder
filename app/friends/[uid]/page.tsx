...
export default function FriendTasksPage() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      const [t, p] = await Promise.all([getViewableTasksByFriend(params.uid, user.uid), getUserProfile(params.uid)])
      setTasks(t)
      setProfile(p)
    })()
  }, [user, params?.uid])
  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-6">
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        {tasks.length === 0 && (
          <div className="text-sm text-muted-foreground col-span-full">No viewable tasks yet.</div>
        )}
      </div>
      {/* </CHANGE> */}
    </main>
